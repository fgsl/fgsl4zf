...
{	
	...
	{		
		...
		foreach($this->_options[self::DATA] as $fieldName => $dado)
		{
			...
			try {
				...
				if ($typeElement == Fgsl_Form_Constants::SELECT)
				{
					...
					$selectOptions = $model->getOptions($fieldName);					
		...
		$formType = $this->_options[self::DATA][$model->getFieldKey()] === '' ? 'insert' : 'edit';
